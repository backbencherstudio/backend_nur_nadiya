...
export const addBioData = async (req, res) => {
    try {
        ...
        const { full_name, date_of_birth, place_of_birth, nationality, height, weight, name_of_airPort, marital_status, religion, education_level, age_of_childern, number_of_childern, allergies, physical_disabilities, mental_illness, epilepsy, tuberculosis, heart_disease, malaria, operations, others, dietary_restrictions, preference_for_rest_days, any_other_remarks, care_of_infants, care_of_infants_willingness, care_of_infants_experience, care_of_infants_assessment, care_of_elderly, care_of_elderly_willingness, care_of_elderly_experience, care_of_elderly_assessment, care_of_disabled, care_of_disabled_willingness, care_of_disabled_experience, care_of_disabled_assessment, general_housework, general_housework_willingnes, general_housework_experience, general_housework_assessment, cooking, cooking_willingness, cooking_experience, cooking_assessment, language_abilities, language_abilities_willingness, language_abilities_experience, language_abilities_assessment, other_skills, other_skills_willingness, other_skills_experience, other_skills_assessment, date_from, date_to, country, employer, work_duties, remarks, other_remarks, } = req.body;
...
export const getBioDataList = async (req, res) => {
    try {
        // Extract query parameters
        const { search = '', status = '', nationality = '', date_from = '', date_to = '', page = '1', limit = '10' } = req.query;
        ...
        if (search) {
            whereClause.OR = [
                ...
                { other_skills: { contains: search, mode: 'insensitive' } },
                { language_abilities: { contains: search, mode: 'insensitive' } }
            ];
        }
        // Status filter
        ...
        if (date_from || date_to) {
            ...
            if (date_from) {
                whereClause.date_of_birth.gte = new Date(date_from);
            }
            if (date_to) {
                whereClause.date_of_birth.lte = new Date(date_to);
            }
        }
...
export const changeBioStatus = async (req, res) => {
    try {
        const { id } = req.params;
...
export const editBioDataById = async (req, res) => {
    try {
        const { id } = req.params;
